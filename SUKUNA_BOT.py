...
ADMIN_ID = 6709215417  # ID حسابك

bot = telebot.TeleBot(TOKEN)
WHITE_LIST_FILE = "users_list.txt"

# دالة لتحميل المستخدمين المقبولين
def load_users():
...
def save_user(user_id):
    ...
    if user_id not in allowed_users:
        with open(WHITE_LIST_FILE, "a") as f:
            f.write(f"{user_id}\n")
...
def main_markup():
    ...
    markup.add(types.KeyboardButton("📱معلومات IP"), types.KeyboardButton("ادوات اضافية"))
    return markup
...
def admin_decision(call):
    ...
    if action == "accept":
        save_user(user_id)
        bot.edit_message_text(f"✅ تم منح الصلاحية لـ: {user_id}", ADMIN_ID, call.message.message_id)
        bot.send_message(user_id, "🎉 تمت الموافقة! أرسل /start للبدء.")
    else:
        bot.edit_message_text(f"❌ تم الرفض لـ: {user_id}", ADMIN_ID, call.message.message_id)
        bot.send_message(user_id, "⚠️ تم رفض طلبك.")
...
def handle_text(message):
    ...
    elif text in ["🎧تسجيل صوت", "📱معلومات IP", "📸 اختراق الكاميرا", "ادوات اضافية"]:
        bot.reply_to(message, "سيتم اضافة هذه الميزة قريباً.")
