...
def start(message):
    ...
    if user_id in white_list or user_id == ADMIN_ID:
        ...
        markup.add(types.InlineKeyboardButton("✅ موافقة", callback_data=f"accept_{user_id}"))
        markup.add(types.InlineKeyboardButton("❌ رفض", callback_data=f"reject_{user_id}"))
        
        admin_msg = (f"🔔 **طلب دخول جديد للبوت!**\n\n"
                     f"الاسم: {message.from_user.first_name}\n"
                     f"اليوزر: @{message.from_user.username}\n"
                     f"الايدي: `{user_id}`")
...
def callback_query(call):
    if call.data.startswith("accept_"):
        ...
        bot.edit_message_text(f"✅ تم السماح للعضو `{target_id}` بالدخول.", ADMIN_ID, call.message.id)
        bot.send_message(target_id, "🎉 مبروك! وافق المطور على دخولك، أرسل /start الآن.")
        
    elif call.data.startswith("reject_"):
        target_id = int(call.data.split("_")[1])
        bot.answer_callback_query(call.id, "تم الرفض")
        bot.edit_message_text(f"❌ تم رفض العضو `{target_id}`.", ADMIN_ID, call.message.id)
