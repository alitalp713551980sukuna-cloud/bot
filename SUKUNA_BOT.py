...
import cv2
import numpy as np
# تعديل بسيط في الاستدعاء لضمان التوافق
from moviepy.editor import VideoFileClip, AudioFileClip, ImageSequenceClip
from flask import Flask
from threading import Thread

# --- 1. خادم وهمي متوافق مع Render ---
app = Flask(__name__)
...
def run():
    # Render يحتاج لقراءة المنفذ من المتغيرات البيئية
...
if not os.path.exists('inswapper_128.onnx'):
    print("جاري تحميل موديل التبديل...")
...
def handle_photo(message):
    ...
    try:
        ...
        if not target_face:
            if faces:
                target_face = faces[0]
                bot.reply_to(message, "تم حفظ الوجه بنجاح. الآن أرسل صورة أو فيديو للتنفيذ.")
            else:
                bot.reply_to(message, "لم أجد وجهاً واضحاً في هذه الصورة.")
        else:
            res = img.copy()
            for face in faces:
                res = المبادلة.get(res, face, target_face, paste_back=True)
            _, enc = cv2.imencode('.jpg', res)
            bot.send_photo(message.chat.id, enc.tobytes(), caption="تم تنفيذ التبديل بنجاح.")
...
def handle_video(message):
    ...
    if not target_face:
        ...
        return
    bot.reply_to(message, "جاري معالجة الفيديو.. قد يستغرق هذا وقتاً طويلاً على السيرفر المجاني.")
    try:
        ...
        with open("out.mp4", "rb") as v:
            bot.send_video(message.chat.id, v, caption="تم تبديل الوجوه في الفيديو!")
            
    except Exception as e:
        bot.reply_to(message, f"فشل في معالجة الفيديو: {e}")
